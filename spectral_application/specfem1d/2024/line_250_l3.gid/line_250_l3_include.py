...
from KratosMultiphysics.SpectralApplication import *
...
class Model:
    def __init__( self, problem_name, path, results_path, logging=True ):
        ...
        ## generating solver
        import explicit_spectral_strategy
        self.solver = explicit_spectral_strategy.SampleSolver( self.model_part, self.abs_tol, self.rel_tol, self.analysis_parameters )
    ...
    def AddDofsForNodes(self, nodes):
        import explicit_spectral_strategy
        explicit_spectral_strategy.AddDofsForNodes( nodes )

    def AddDofs(self, model_part):
        import explicit_spectral_strategy
        explicit_spectral_strategy.AddDofs( model_part )

    def AddDofsForNode(self, node):
        import explicit_spectral_strategy
        explicit_spectral_strategy.AddDofsForNode( node )

    def AddVariables(self, model_part):
        import explicit_spectral_strategy
        explicit_spectral_strategy.AddVariables( model_part )
    ...
    def SetModelPart(self, model_part):
        ...
        ## generating solver
        import explicit_spectral_strategy
        self.solver = explicit_spectral_strategy.SolverAdvanced( self.model_part, self.domain_size, number_of_time_steps, self.analysis_parameters, self.abs_tol, self.rel_tol )
        (self.solver).CalculateReactionFlag = False
        ##################################################################
        ## ADD DOFS ######################################################
        ##################################################################
    ...
    def InitializeModel( self ):
        ...
        nu = 0.3
        rho = 2.5e3
        self.model_part.Properties[1].SetValue(YOUNG_MODULUS, mu*2*(1+nu))
        self.model_part.Properties[1].SetValue(POISSON_RATIO, nu)
        self.model_part.Properties[1].SetValue(DENSITY, rho)
        self.model_part.Properties[1].SetValue(INTEGRATION_ORDER, 3)
        print("Material is assigned to the bar")
        ##################################################################
        ## ACTIVATION ####################################################
        ##################################################################
