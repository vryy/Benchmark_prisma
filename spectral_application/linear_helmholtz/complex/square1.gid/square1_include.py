...
class Model:
    def __init__( self, problem_name, path, results_path, logging=True, solver="mkl-pardiso" ):
        ...
        if solver == "mkl-pardiso":
            plinear_solver = MKLComplexPardisoSolver()
        elif solver == "skyline-lu":
    ...
    def AddVariables(self, model_part):
        model_part.AddNodalSolutionStepVariable(COMPLEX_PRESSURE)
        model_part.AddNodalSolutionStepVariable(COMPLEX_TEMPERATURE)

    def AddDofsForNodes(self, nodes):
        for node in nodes:
            node.AddDof(COMPLEX_PRESSURE, COMPLEX_TEMPERATURE)
    ...
    def SetOutputPath(self, path=os.getcwd()+"/", name="square1"):
        self.results_path = path
        self.problem_name = name
        self.gid_io = ComplexSDGidPostIO( self.results_path+name, self.post_mode, self.multi_file_flag, self.write_deformed_flag, self.write_elements )
    ...
    def WriteOutput( self, time ):
        ...
        self.gid_io.WriteNodalResults(COMPLEX_PRESSURE, time, 0)
        print("nodal COMPLEX_PRESSURE written")
    ...
    def InitializeModel( self ):
        ...
        self.model_part.Properties[1].SetValue(WAVE_NUMBER,      1.0 )
