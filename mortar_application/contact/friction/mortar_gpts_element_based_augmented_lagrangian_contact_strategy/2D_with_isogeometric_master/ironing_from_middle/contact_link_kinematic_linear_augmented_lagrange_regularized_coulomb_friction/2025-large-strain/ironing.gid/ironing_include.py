...
import mortar_gpts_element_based_augmented_lagrangian_contact_strategy
...
class Model:
    def __init__( self, problem_name, path, results_path, logging=True ):
        ...
        ## generating solver
        self.solver = mortar_gpts_element_based_augmented_lagrangian_contact_strategy.SampleSolver(self.model_part, self.abs_tol, self.rel_tol, self.analysis_parameters)
    ...
    def AddVariables(self, model_part):
        mortar_gpts_element_based_augmented_lagrangian_contact_strategy.AddVariables( model_part )

    def AddDofs(self, model_part):
        mortar_gpts_element_based_augmented_lagrangian_contact_strategy.AddDofs( model_part )
    ...
    def AddDofsForNodes(self, nodes):
        mortar_gpts_element_based_augmented_lagrangian_contact_strategy.AddDofsForNodes( nodes )
    ...
    def WriteOutput( self, time ):
        self.DeactivateConditions('isogeometric_master')
        #
        ...
        self.gid_io.WriteNodalResults(LAMBDA, time, 0)
        print("nodal LAMBDA written")
        ...
        self.gid_io.Reset()
        #
        self.ReactivateConditions('isogeometric_master')
    ...
    def InitializeModel( self ):
        ...
        self.model_part.Properties[1].SetValue(YOUNG_MODULUS,        30000 )
        self.model_part.Properties[1].SetValue(POISSON_RATIO,          0.3 )
        self.model_part.Properties[1].SetValue(THICKNESS,            1 )
        if self.material == "st-venant-kirchhoff":
            self.model_part.Properties[1].SetValue(CONSTITUTIVE_LAW, StVenantKirchhoff_PlaneStrain() )
            print("StVenantKirchhoff_PlaneStrain model selected for Cylinder, description: Cylinder")
        elif self.material == "neo-hookean":
            self.model_part.Properties[1].SetValue(CONSTITUTIVE_LAW, NeoHookean2D() )
            print("NeoHookean2D model selected for Cylinder, description: Cylinder")
            self.model_part.Properties[2].SetValue(DENSITY,            0 )
        self.model_part.Properties[2].SetValue(YOUNG_MODULUS,        10000 )
        self.model_part.Properties[2].SetValue(POISSON_RATIO,          0.3 )
        self.model_part.Properties[2].SetValue(THICKNESS,            1 )
        if self.material == "st-venant-kirchhoff":
            self.model_part.Properties[2].SetValue(CONSTITUTIVE_LAW, StVenantKirchhoff_PlaneStrain() )
            print("StVenantKirchhoff_PlaneStrain model selected for Slab, description: Cylinder")
        elif self.material == "neo-hookean":
            self.model_part.Properties[2].SetValue(CONSTITUTIVE_LAW, NeoHookean2D() )
            print("StVenantKirchhoff_PlaneStrain model selected for Slab, description: Cylinder")
        else:
            raise Exception(f"Unknown material {self.material}")
        ##################################################################
        ## MORTAR TYING/CONTACT ##########################################
        ##################################################################
        self.layer_cond_sets["slave"] = []
        self.layer_cond_sets["master"] = []
        for cond in self.model_part.Conditions:
            if cond.Has(MASTER_INDEX):
                slave_index_set = IntegerVector(1)
                slave_index_set[0] = 20 # change to tying
                cond.SetValue(SLAVE_INDEX_SET, slave_index_set)
                cond.SetValue(MASTER_INDEX, -1) # do not involve in contact
                self.layer_cond_sets["master"].append(cond.Id)
            if cond.Has(SLAVE_INDEX):
                slave_index_set = IntegerVector(1)
                slave_index_set[0] = cond.GetValue(SLAVE_INDEX)
                cond.SetValue(SLAVE_INDEX_SET, slave_index_set)
                self.layer_cond_sets["slave"].append(cond.Id)
        ##################################################################
        ## ACTIVATION ####################################################
        ##################################################################
    ...
    def DeactivateConditions(self, layer_cond_name):
        if layer_cond_name in self.layer_cond_sets:
            for cond_id in self.layer_cond_sets[layer_cond_name]:
                cond = self.model_part.Conditions[cond_id]
                cond.Set(ACTIVE, False)

    def ReactivateConditions(self, layer_cond_name):
        if layer_cond_name in self.layer_cond_sets:
            for cond_id in self.layer_cond_sets[layer_cond_name]:
                cond = self.model_part.Conditions[cond_id]
