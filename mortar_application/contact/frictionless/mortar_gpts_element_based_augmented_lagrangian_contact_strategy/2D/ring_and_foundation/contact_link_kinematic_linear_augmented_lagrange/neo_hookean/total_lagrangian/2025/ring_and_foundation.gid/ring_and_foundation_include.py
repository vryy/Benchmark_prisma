...
class Model:
    def __init__( self, problem_name, path, results_path, logging=True ):
        ...
        self.solver = mortar_gpts_element_based_augmented_lagrangian_contact_strategy.SampleSolverEkateQuasiStatic(self.model_part, self.abs_tol, self.rel_tol, self.analysis_parameters)
        mortar_gpts_element_based_augmented_lagrangian_contact_strategy.AddVariables( self.model_part )
        ##################################################################
        ## READ MODELPART ################################################
        ##################################################################
        #reading a model
    ...
    def WriteOutput( self, time ):
        ...
        self.gid_io.WriteNodalResults(LAMBDA, self.model_part.Nodes, time, 0)
        print("nodal LAMBDA written")
        self.gid_io.WriteNodalResults(CONTACT_FORCE, self.model_part.Nodes, time, 0)
        print("nodal CONTACT_FORCE written")
    ...
    def InitializeModel( self ):
        ...
        self.model_part.Properties[1].SetValue(YOUNG_MODULUS,        1e+08 )
        self.model_part.Properties[1].SetValue(POISSON_RATIO,            0 )
        ...
        print("Linear elastic model selected for Foundation, description: Foundation")
        ...
        print("Neo Hookean model selected for Outer_ring, description: Outer_ring")
        self.model_part.Properties[3].SetValue(DENSITY,            0 )
        self.model_part.Properties[3].SetValue(YOUNG_MODULUS,        1e+05 )
        self.model_part.Properties[3].SetValue(POISSON_RATIO,          0.3 )
        self.model_part.Properties[3].SetValue(THICKNESS,            1 )
        self.model_part.Properties[3].SetValue(CONSTITUTIVE_LAW, NeoHookean2D() )
        print("Neo Hookean model selected for Inner_ring, description: Inner_ring")
        ##################################################################
