...
class Model:
    def __init__( self, problem_name, path, results_path, logging=True ):
        ...
        self.solver = mortar_gpts_element_based_active_set_penalty_contact_strategy.SampleSolverEkateQuasiStatic(self.model_part, self.abs_tol, self.rel_tol, self.analysis_parameters)
        mortar_gpts_element_based_active_set_penalty_contact_strategy.AddVariables( self.model_part )
        ##################################################################
        ## READ MODELPART ################################################
        ##################################################################
        #reading a model
