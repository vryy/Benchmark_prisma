...
from KratosMultiphysics.StructuralOptimizationApplication import *
...
##################################################################
import structural_optim_solver
##################################################################
class ConicSolver(structural_optim_solver.ConicSolver):
    def __init__(self):
        structural_optim_solver.ConicSolver.__init__(self)
        self.sigma_Y = 250.0 * math.sqrt(1.5)
        self.sigma_0 = 2.0 / math.sqrt(3.0) * self.sigma_Y
        self.abs_tol = 1e-6
        self.rel_tol = 1e-10

    def CreateConstraint(self, sxx, syy, szz, sxy, syz, sxz):
        return structural_optim_solver.CreateVm2dConstraint(sxx, syy, sxy, self.sigma_0)

    def ValidateConstraint(self, sxx, syy, szz, sxy, syz, sxz):
        vm_stress = math.sqrt((sxx-syy)**2 + (2*sxy)**2)
        return abs(vm_stress - self.sigma_0) < self.abs_tol
...
class Model:
    def __init__( self, problem_name, path, results_path, logging=True ):
        ...
        fricrampfactor = 0.0
        self.analysis_parameters['dimension'] = self.domain_size
        ...
        self.analysis_parameters['decouple_build_and_solve'] = True # must be True for conic solver
        ...
        ## generating solver
        self.solver = structural_optim_solver.Solver( self.model_part, self.analysis_parameters, self.abs_tol, self.rel_tol )
        ...
        self.solver.conic_solver = ConicSolver()
        self.solver.Initialize()
        (self.solver.solver).SetEchoLevel(2)
        (self.solver.solver).max_iter = 0 # to avoid any iteration
    ...
    def AddDofsForNodes(self, nodes):
        import structural_optim_solver
        structural_optim_solver.AddDofsForNodes( nodes )

    def AddDofs(self, model_part):
        import structural_optim_solver
        structural_optim_solver.AddDofs( model_part )

    def AddDofsForNode(self, node):
        import structural_optim_solver
        structural_optim_solver.AddDofsForNode( node )

    def AddVariables(self, model_part):
        import structural_optim_solver
        structural_optim_solver.AddVariables( model_part )
    ...
    def InitializeModel( self ):
        ...
        self.model_part.Properties[1].SetValue(YOUNG_MODULUS,      2e7 )
        ...
        print("Linear elastic model selected for the bulk")
        ##################################################################
        ## ACTIVATION ####################################################
        ##################################################################
