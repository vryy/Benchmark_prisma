...
def main(output=False, logging=False):
    ...
    for node in prescribed_nodes:
        node.SetSolutionStepValue(TEMPERATURE, 350.0)
...
def test():
    ...
    ### pytesting results
    tol = 1e-06
    assert(abs(model.model_part.Nodes[4].GetSolutionStepValue(DISPLACEMENT_X) - 0.000650) < 1e-10)
    assert(abs(model.model_part.Nodes[4].GetSolutionStepValue(DISPLACEMENT_Y) - 0.000650) < 1e-10)
    assert(abs(model.model_part.Nodes[4].GetSolutionStepValue(DISPLACEMENT_Z) - 0.000650) < 1e-10)
    print("Test passed")
