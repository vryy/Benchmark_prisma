...
alpha = 100.0
beta = 100.0
r = E*A*(H**3)/3/math.sqrt(3)/(L1**3)

def SetLoad(model_part, factor):
    model_part.Nodes[3].SetSolutionStepValue(FORCE_X, 0.0)
    model_part.Nodes[3].SetSolutionStepValue(FORCE_Y, -factor*r)

def main(output=True, logging=True):
    model = two_beams_include.Model('two_beams',os.getcwd()+"/",os.getcwd()+"/", logging)
    ...
    for i in range(0, nsteps):
        ...
        if output:
            ...
            ifile.write(str(time) + '\t' + str(ux/B2) + '\t' + str(uy/H) + '\n')
    ...
    ux = model.model_part.Nodes[1].GetSolutionStepValue(DISPLACEMENT_X)
    uy = model.model_part.Nodes[3].GetSolutionStepValue(DISPLACEMENT_Y)
    ref_ux = 0.0280700232954
    ref_uy = -2.03588495553
