...
def main(output=True, logging=True):
    ...
    for i in range(0, nsteps):
        ...
        for node in prescribed_nodes:
            node.SetSolutionStepValue(DISPLACEMENT_Y, disp) # for first order element it is ok, but for second order element, this displacement setting scheme would lead to nonrobust convergence
...
def test():
    ...
    ref_reac = 2.876691764955273e+04
    reac = 0.0
    for node in prescribed_nodes:
