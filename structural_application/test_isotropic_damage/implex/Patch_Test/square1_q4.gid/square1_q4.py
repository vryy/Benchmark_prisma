...
from square1_q4_include import *
model_name_ = 'square1_q4'
##################################################################
###  SIMULATION  #################################################
start_time = time_module.time()
##################################################################

def WriteLog(ifile, ifile2, step, disp, elem, reaction_nodes, process_info):
    ...
    ifile2.write(str(step+1) + "\t" + str(disp) + "\t" + str(reac_x) + "\n")
    ifile2.flush()

def main(logging=True, output=True, output_last=False, delta_disp = 1e-6, nsteps_phase = [150, 50, 150]):
    model = simulation_include.Model('square1_q4',os.getcwd()+"/",os.getcwd()+"/",logging=logging)
    ...
    for step in range(0, nsteps_phase[0]):
        ...
        if logging:
            WriteLog(ifile, ifile2, step, disp, model.model_part.Elements[1], reaction_nodes, model.model_part.ProcessInfo)
...
if __name__ == '__main__':
    if len(sys.argv) > 1:
        ...
        main(logging=True, output=False, output_last=True, delta_disp=1e-6, nsteps_phase=[150, 50, 150])
