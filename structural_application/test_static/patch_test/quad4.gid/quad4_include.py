...
class Model:
    ...
    def SetModelPart(self, model_part):
        ...
        ## generating solver
        import structural_solver_static
        self.solver = structural_solver_static.SolverAdvanced( self.model_part, self.domain_size, number_of_time_steps, self.analysis_parameters, self.abs_tol, self.rel_tol )
        (self.solver).CalculateReactionFlag = True
        ##################################################################
        ## ADD DOFS ######################################################
        ##################################################################
