...
def main(output=True, logging=True, penalty=1e12):
    ...
    tying_util.SetValue(MAXIMAL_DETECTION_DISTANCE, 1e-3)
    tying_util.SetValue(GAP_TOLERANCE, 1e-3)
    tying_util.SetValue(TYING_INTEGRATION_ORDER, 3)
    tying_util.InitializeSearchTree(model.model_part, 10)
    mortar_links = tying_util.SetupTyingLinkElementBased(model.model_part, 10, "tying_link_kinematic_linear_penalty")
