...
def WriteNode(ifile, step, time, node):
    ...
    vx = node.GetSolutionStepValue(DISPLACEMENT_NULL_DT_X)
    vy = node.GetSolutionStepValue(DISPLACEMENT_NULL_DT_Y)
    vz = node.GetSolutionStepValue(DISPLACEMENT_NULL_DT_Z)
    ax = node.GetSolutionStepValue(ACCELERATION_NULL_X)
    ay = node.GetSolutionStepValue(ACCELERATION_NULL_Y)
    az = node.GetSolutionStepValue(ACCELERATION_NULL_Z)
...
def main(output=True, logging=True):
    ...
    model = beam_hex8_include.Model('beam_hex8',os.getcwd()+"/",os.getcwd()+"/",logging=False,option=1)
    ...
    dynmodel = beam_hex8_include.Model('beam_hex8',os.getcwd()+"/",os.getcwd()+"/",logging=logging,option=2)
    ...
    if logging:
        ...
        WriteNode(ifile, step, time - 2*delta_time, tip_node)

    ## create a second initial step
    step += 1
    time += delta_time
    ...
    # nsteps = 1000
    nsteps = int(100.0/delta_time)
    # nsteps = int(1.0/delta_time)
    # nsteps = 1
...
def test():
    ...
    print("vy: %.16e" % vy)
    print("ay: %.16e" % ay)
    ref_dy = 3.9153385181763542e-03
    ref_vy = -1.6073026656764616e-02
    ref_ay = -2.9694862732668168e-02
