...
class Model:
    def __init__( self, problem_name, path, results_path, logging=True ):
        ...
        ## DEFINE MODELPART ##############################################
        ##################################################################
        self.model_part = GComplexModelPart(problem_name)
        self.path = path
        ...
        ##################################################################
        #reading a model
        self.model_part_io = GComplexModelPartIO(self.path+self.problem_name)
        ...
        self.gid_io = GComplexSDGidPostIOReal( self.results_path+self.problem_name, self.post_mode, self.multi_file_flag, self.write_deformed_flag, self.write_elements )
        ...
        plinear_solver = GComplexSkylineLUFactorizationSolver()
    ...
    def SetModelPart(self, model_part):
        ...
        ##################################################################
        #defining linear solver
        plinear_solver = MKLGComplexPardisoSolver()
        self.solver.structure_linear_solver = plinear_solver
        self.solver.Initialize()
        (self.solver.solver).SetEchoLevel(2)
    ...
    def SetOutputPath(self, path):
        ...
        self.gid_io = GComplexSDGidPostIO( self.results_path+self.problem_name, self.post_mode, self.multi_file_flag, self.write_deformed_flag, self.write_elements )
    ...
    def InitializeModel( self ):
        ...
        self.model_part.Properties[1].SetValue(GCOMPLEX_CONSTITUTIVE_LAW, GComplexIsotropic3D() )
        print("Linear elastic model selected, description: GComplexIsotropic3D")
        ##################################################################
        ## ACTIVATION ####################################################
        ##################################################################
