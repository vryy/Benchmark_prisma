...
class Model:
    def __init__( self, problem_name, path, results_path, logging=True ):
        ...
        # self.analysis_parameters['solution_strategy'] = "explicit"
        ...
        self.vtk_io = VtkVTMIO( self.path+self.problem_name, vtk_post_mode )
    ...
    def WriteOutput( self, time ):
        ...
        self.gid_io.WriteNodalResults(TEMPERATURE, time, 0)
        print("nodal TEMPERATURE written")
        self.gid_io.WriteNodalResults(ENTROPY, time, 0)
        print("nodal ENTROPY written")
        ...
        self.vtk_io.RegisterNodalResults(TEMPERATURE, 0)
        self.vtk_io.RegisterNodalResults(ENTROPY, 0)
    ...
    def InitializeModel( self ):
        ...
        self.model_part.Properties[1].SetValue(DENSITY,         8960 ) # 8960 kg/m^3
        self.model_part.Properties[1].SetValue(YOUNG_MODULUS,      117e9 ) # 117 GPa
        self.model_part.Properties[1].SetValue(POISSON_RATIO,          0.35 )
        self.model_part.Properties[1].SetValue(THICKNESS, 1.0 )
        self.model_part.Properties[1].SetValue(CONSTITUTIVE_LAW, PlaneStrain() )
        self.model_part.Properties[1].SetValue(THERMAL_CONDUCTIVITY, 385.0 ) # 385 W/(m K)
        self.model_part.Properties[1].SetValue(THERMAL_EXPANSION, 16.5e-6 )
        self.model_part.Properties[1].SetValue(SPECIFIC_HEAT_CAPACITY, 385.0 ) # 385 J/(kg K)
        aux_util.SetValueForProperties(self.model_part.Properties[1], HEAT_SOURCE,    NullHeatSource() )
        print("Linear elastic model selected for Copper, description: copper")
        self.model_part.Properties[2].SetValue(DENSITY,         8960 ) # 8960 kg/m^3
        self.model_part.Properties[2].SetValue(YOUNG_MODULUS,      117e9 ) # 117 GPa
        self.model_part.Properties[2].SetValue(POISSON_RATIO,          0.35 )
        self.model_part.Properties[2].SetValue(THICKNESS, 1.0 )
        self.model_part.Properties[2].SetValue(CONSTITUTIVE_LAW, PlaneStrain() )
        self.model_part.Properties[2].SetValue(THERMAL_CONDUCTIVITY, 385.0 ) # 385 W/(m K)
        self.model_part.Properties[2].SetValue(THERMAL_EXPANSION, 16.5e-6 )
        self.model_part.Properties[2].SetValue(SPECIFIC_HEAT_CAPACITY, 385.0 ) # 385 J/(kg K)
        aux_util.SetValueForProperties(self.model_part.Properties[2], HEAT_SOURCE,    NullHeatSource() )
        print("Linear elastic model selected for Aluminium, description: copper")
        # self.model_part.Properties[2].SetValue(DENSITY,         2700 ) # 2700 kg/m^3
        # self.model_part.Properties[2].SetValue(YOUNG_MODULUS,      75e9 ) # 75 GPa
        # self.model_part.Properties[2].SetValue(POISSON_RATIO,          0.34 )
        # self.model_part.Properties[2].SetValue(THICKNESS, 1.0 )
        # self.model_part.Properties[2].SetValue(CONSTITUTIVE_LAW, PlaneStrain() )
        # self.model_part.Properties[2].SetValue(THERMAL_CONDUCTIVITY, 230.0 ) # 205-250 W/(m K)
        # self.model_part.Properties[2].SetValue(THERMAL_EXPANSION, 23.1e-6 )
        # self.model_part.Properties[2].SetValue(SPECIFIC_HEAT_CAPACITY, 897.0 ) # 897 J/(kg K)
        # aux_util.SetValueForProperties(self.model_part.Properties[2], HEAT_SOURCE,    NullHeatSource() )
        # print("Linear elastic model selected for Aluminium, description: aluminium")
        ##################################################################
        ## ACTIVATION ####################################################
        ##################################################################
