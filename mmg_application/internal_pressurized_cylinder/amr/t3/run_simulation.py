...
def create_params(output=True, stress_recovery_type=0, neighbour_expansion_level=1, refine_vector=[], hausdorff_distance=20.0):
    params = {}
    ...
    params["initial_refinement_process"] = mmg_simulator.MmgRefinementProcessBasedOnRefineVector(refine_vector)
    ###############
    return params

def main(name="mesh10x10", logging=True, output=True, time=1.0, refine_vector=[], stress_recovery_type=0, neighbour_expansion_level=1):
    ...
    for node in model.model_part.Nodes:
        node.SetSolutionStepValue(NODAL_MMG_SCALAR_METRIC, 20.0)
...
def study(output=True, logging=True, ratio=0.1, nrefine=8, initial_mesh_size=20.0):
    name = "mesh10x10"
    mdpa_path = os.getcwd() + '/../../design_data/' + name + '.gid/'
    ...
    params = create_params(output=output, stress_recovery_type=1, neighbour_expansion_level=2, refine_vector=[], hausdorff_distance=initial_mesh_size)
...
def study1(output=True, logging=True, ratio=0.1, nrefine=12, initial_mesh_size=20.0):
    name = "mesh10x10"
    mdpa_path = os.getcwd() + '/../../design_data/' + name + '.gid/'
    model = meshxx_mmg_include.Model(name,mdpa_path,os.getcwd()+"/",logging=False)
    model.InitializeModel()
    ...
    for step in range(0, nrefine):
        ...
        if step < nrefine-1:
            ...
            e_vec = []
            stress_util = RecoverStressUtility()
            stress_util.ComputeKellyErrorEstimation2D(model.model_part, DISPLACEMENT)
            for elem in model.model_part.Elements:
...
def test():
    sim, model = study(output=False, logging=False, ratio=0.1, nrefine=3, initial_mesh_size=20.0)
    l2_error_ref = 2.9640258965966965e-03
    h1_error_ref = 7.0586210121916515e-02
    h1_error_rs_ref = 7.1415774612624402e-03
