...
sys.path.append("../infinite_plate_with_hole")
...
def save(order=[2, 2], nsampling=10):
    ...
    serializer1 = Serializer("mpatch", SerializerTraceType.SERIALIZER_NO_TRACE) # use SERIALIZER_TRACE_ERROR to save the restart in Ascii (useful for debugging)
    mpatch.Save(serializer1, "mpatch")

    return mpatch

def load():
    serializer2 = Serializer("mpatch", SerializerTraceType.SERIALIZER_NO_TRACE)
    mpatch = MultiPatch2D()
...
def main(logging=True, output=True, order=[2, 2], nsampling=10):
    ...
    if output:
        ...
        # print(f"mpatch_post: {mpatch_post}")
