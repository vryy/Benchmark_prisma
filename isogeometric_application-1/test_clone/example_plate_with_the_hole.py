...
def main(logging=True, output=True, order=[2, 2], nsampling=10):
    ...
    mpatch_compute = mpatch.Clone()
    print(mpatch_compute)

    mpatch_compute.Enumerate()

    if logging:
        mpatch_export.Export(mpatch_compute, "example_plate_with_hole.m")

    mpatch_mp = CreateModel(mpatch_compute)
    model_part = mpatch_mp.GetModelPart()
    ...
    if output:
        ...
        mpatch_post = mpatch_compute.Clone()
