...
def CreateMultiPatch(configuration=1):

    print("CreateMultiPatch for configuration %d" % (configuration))

    rec1_ptr = geometry_factory.CreateRectangle([0.0, 0.0, 0.0], [0.5*L, h, 0.0])
    rec1 = rec1_ptr.GetReference()
    rec1.Id = 1
    rec1.LayerIndex = 1

    rec2_ptr = geometry_factory.CreateRectangle([0.5*L, 0.0, 0.0], [L, h, 0.0])
    rec2 = rec2_ptr.GetReference()
    rec2.Id = 2
    rec2.LayerIndex = 1

    ######create multipatch
    if configuration == 1:
        mpatch = MultiPatch2D([rec1_ptr, rec2_ptr])
        bsplines_patch_util.MakeInterface(rec1, BoundarySide2D.U1, rec2, BoundarySide2D.U0, BoundaryDirection.Forward)
    elif configuration == 2:
        bsplines_patch_util.Reverse(rec2, 1)
        bsplines_patch_util.Reverse(rec2, 0)

        mpatch = MultiPatch2D([rec1_ptr, rec2_ptr])
        bsplines_patch_util.MakeInterface(rec1, BoundarySide2D.U1, rec2, BoundarySide2D.U1, BoundaryDirection.Reversed)
    else:
        raise Exception("Invalid configuration %d" % (configuration))

    return mpatch

def Refine(mpatch, nsampling_u=40, nsampling_v=5):
    print("###############REFINEMENT###############")
##    patch1_ptr = mpatch[1]
    multipatch_refine_util.DegreeElevate(mpatch[1], [1, 1])
    multipatch_refine_util.DegreeElevate(mpatch[2], [1, 0])
...
def main(output=True, logging=True, configuration=1, nsampling_u=40, nsampling_v=5):
    mpatch = CreateMultiPatch(configuration=configuration)
    mpatch = Refine(mpatch, nsampling_u=nsampling_u, nsampling_v=nsampling_v)
    mpatch.Enumerate()
...
def test():
    for configuration in [1, 2]:
        model = main(logging = False, output = False, configuration=configuration, nsampling_u=40, nsampling_v=5)
...
if __name__ == '__main__':
    if len(sys.argv) > 1:
        ...
        main(logging=True, output=True, configuration=2)
