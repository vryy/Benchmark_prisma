...
def ConstructModelPart(p4est_model, order):
    ...
    for hnode in hanging_nodes:
        ...
        constraint_id = constraint_id + 1
        p4est_model.CreateConstraint(hnode, constraint_id, constraint_name, DISPLACEMENT_Z)
        constraint_id = constraint_id + 1
...
def Solve(model, p4est_model, time, output=True):
    ...
    if output:
        ...
        p4est_model.ExportVTK("cube", time, True, True)
...
def main(logging=True, output=True, parallel=False):
    ...
    p4est_quad_int.Register(STRESSES, 6)
    ...
    # mark and refine
    p4est_model.Mark([15], P4estRefineFlag.TO_REFINE)
    p4est_model.Refine()
    p4est_model.Repartition()
...
if __name__ == '__main__':
    if len(sys.argv) > 1:
        ...
        main(logging=True, output=True, parallel=True)
