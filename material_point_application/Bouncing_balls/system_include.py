...
##################################################################
class Model:
    def __init__( self, problem_name, path, results_path, nsampling, order = 1 ):
        ##################################################################
        ## DEFINE MODELPART ##############################################
        ##################################################################
        ...
        self.analysis_parameters['dimension'] = 2
        ...
        self.analysis_parameters['log_residuum'] = False
        ...
        self.solver = structural_solver_advanced.SolverAdvanced( self.model_part, 2, 1, self.analysis_parameters, self.abs_tol, self.rel_tol )
        ...
        if order == 1:
            ...
            element_type = 3 # Q9
        mesh_util.GenerateStructuredModelPart(self.model_part, start_point, end_point, nsampling[0], nsampling[1], element_name, element_type, self.model_part.Properties[1])

        # print("nodes:")
        # for node in self.model_part.Nodes:
        #     print("node " + str(node.Id) + ": " + str(node.X0) + ", " + str(node.Y0) + ", " + str(node.Z0))
        # sys.exit(0)
    ...
    def AddVariables(self, model_part):
        ...
        mpm_explicit_strategy.AddVariables( model_part )
    ...
    def WriteOutput( self, time ):
        ...
        self.gid_io.WriteNodalResults(NODAL_NORMAL_VECTOR, time, 0)
        print("nodal NODAL_NORMAL_VECTOR written")
    ...
    def InitializeModel( self ):
        ...
        ## ELEMENTS on layers ############################################
        self.layer_sets = {}
        self.layer_sets['soil'] = []
        for elem in self.model_part.Elements:
            self.layer_sets['soil'].append(elem.Id)
        # ## NODES on layers ###############################################
        ...
        g_vect[1] = 0.0
        g_vect[2] = 0.0
        self.model_part.Properties[1].SetValue(DENSITY,       1.0e3 )
        self.model_part.Properties[1].SetValue(BODY_FORCE,       ZeroVector(3) )
        self.model_part.Properties[1].SetValue(GRAVITY,       g_vect )
        self.model_part.Properties[1].SetValue(YOUNG_MODULUS,        1.0e3 )
        self.model_part.Properties[1].SetValue(POISSON_RATIO,         0.3 )
        self.model_part.Properties[1].SetValue(INTEGRATION_ORDER,            3 )
        ...
        if self.material == "plane_stress":
            ...
            print("Plane stress model selected")
        elif self.material == "plane_strain":
            self.model_part.Properties[1].SetValue(CONSTITUTIVE_LAW, PlaneStrain() )
            print("Plane strain model selected")
        else:
            raise Exception("Invalid material " + str(self.material))
        ##################################################################
        ## ACTIVATION ####################################################
        ##################################################################
