...
##################################################################
##################################################################
sys.path.append('./terzaghi_quad9.gid')
import terzaghi_quad9_include
from terzaghi_quad9_include import *

def main(output=True, logging=True):
    model = terzaghi_quad9_include.Model('terzaghi_quad9',os.getcwd()+"/",os.getcwd()+"/",logging)
    ...
    for node in model.model_part.Nodes:
        ...
        if abs(node.Y0 - 7.0) < tol:
            top_nodes.append(node)

    ## logging
    if logging:
        for node in model.model_part.Nodes:
            if abs(node.X0-1.0) < tol and abs(node.Y0-7.0) < tol:
                mon_uy_node = node
            if abs(node.X0-1.0) < tol and abs(node.Y0-0.0) < tol:
                mon_wp_node = node

        ifile = open("displacement.log", "w")
        ifile.write("time\tuy\n")
        ifile1 = open("water_pressure.log", "w")
        ifile1.write("time\twp\n")
        def Record(time):
            ifile.write("%.6e\t%.10e\n" % (time, mon_uy_node.GetSolutionStepValue(DISPLACEMENT_Y)))
            ifile1.write("%.6e\t%.10e\n" % (time, mon_wp_node.GetSolutionStepValue(WATER_PRESSURE)))
    ...
    for node in top_nodes:
        ...
        node.SetSolutionStepValue(FACE_LOAD_Y, pressure)
...
def test():
    ...
    for node in model.model_part.Nodes:
        if abs(node.Y0 - 7.0) < tol:
            top_nodes.append(node)
    disp_y = top_nodes[0].GetSolutionStepValue(DISPLACEMENT_Y)
    ref_disp_y = -6.4968886978e-04
    print("%.10e" % disp_y)
    assert(abs((disp_y - ref_disp_y) / ref_disp_y) < 1e-10)
