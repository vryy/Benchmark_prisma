...
#        self.model_part.Properties[1].SetValue(YOUNG_MODULUS,      1.0e7 )
#        self.model_part.Properties[1].SetValue(POISSON_RATIO,          0.48 )
#        self.model_part.Properties[1].SetValue(THICKNESS, 1.0 )
#        self.model_part.Properties[1].SetValue(CONSTITUTIVE_LAW, PlaneStrain() )
#        self.material_type = "el"
        ...
        self.model_part.Properties[1].SetValue(POISSON_RATIO,          0.48 )
        self.model_part.Properties[1].SetValue(THICKNESS, 1.0 )
        self.model_part.Properties[1].SetValue(TENSILE_STRENGTH, 848.7 )
        self.model_part.Properties[1].SetValue(ISOTROPIC_HARDENING_MODULUS, 0.0)#1.0)#100.0e8 )
        self.model_part.Properties[1].SetValue(INTEGRATION_ORDER, 2 )

        if self.material_type == "j2":
            self.model_part.Properties[1].SetValue(CONSTITUTIVE_LAW, VonMises3dImplicitDC() )
            print("J2 (backward Euler) model selected, description: steel")

#        self.model_part.Properties[1].SetValue(CONSTITUTIVE_LAW, VonMises3dImplicitForwardEuler() )
#        self.material_type = "j2-fe"
#        print("J2 (forward Euler) model selected, description: steel")

        elif self.material_type == "j2-implex":
            self.model_part.Properties[1].SetValue(CONSTITUTIVE_LAW, VonMises3dImplicitExplicit() )
            print("J2 (implex) model selected, description: steel")

        elif self.material_type == "j2-implex-recast":
            self.model_part.Properties[1].SetValue(CONSTITUTIVE_LAW, VonMises3dImplicitExplicitLinearFlow() )
            print("J2 (implex, recast) model selected, description: steel")

        elif self.material_type == "j2-implex-linearized-flow":
            self.model_part.Properties[1].SetValue(CONSTITUTIVE_LAW, VonMises3dImplicitExplicitLinearizedFlow() )
            print("J2 (implex, linearized flow) model selected, description: steel")

        elif self.material_type == "j2-implex-elastic":
            self.model_part.Properties[1].SetValue(CONSTITUTIVE_LAW, VonMises3dImplicitExplicitLinearElastic() )
            print("J2 (implex, linear elastic) model selected, description: steel")

        elif self.material_type == "tr":
            self.model_part.Properties[1].SetValue(TENSILE_STRENGTH, 980.0 ) # approximately 2/sqrt(3) times higher than J2 tensile_strength
            self.model_part.Properties[1].SetValue(CONSTITUTIVE_LAW, Tresca3dImplicitDC() )
            print("Tresca model selected, description: steel")
        ##################################################################
        ## ACTIVATION ####################################################
        ##################################################################
