...
#        print("Linear elastic model selected for Isotropic3D, description: Isotropic3D"
        ...
        # self.model_part.Properties[1].SetValue(ISOTROPIC_HARDENING_MODULUS, 1.0e6 )
        ...
        self.model_part.Properties[1].SetValue(CONSTITUTIVE_LAW, VonMisesG3dImplicitSubstepping() )
        driver = SingleElementTestDriver(2)
        # hlaw = LinearHardeningLaw(848.7, 5.0e6)
        c0 = 848.7
        cinf = 1.0e3
        E = self.model_part.Properties[1].GetValue(YOUNG_MODULUS)
        rho = E/(cinf-c0)
        hlaw = ExponentialHardeningLaw(c0, cinf, rho)
        driver.SetValue(ISOTROPIC_HARDENING_LAW, hlaw, self.model_part.Properties[1])
        self.material_type = "j2-im"
        print("J2 model (implicit) is selected")
        ##################################################################
        ## ACTIVATION ####################################################
        ##################################################################
