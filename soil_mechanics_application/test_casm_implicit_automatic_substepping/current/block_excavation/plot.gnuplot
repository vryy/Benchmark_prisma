...
#plot pc0*t, M*pc0*t*((-log(t)/log(R))**(1.0/N)) title "initial yield envelope", \
#     pck*t, M*pck*t*((-log(t)/log(R))**(1.0/N)) title "later yield envelope", \
#     t*3e1, M*t*3e1 title "critical state line", \
#     p0+(pTr-p0)*t, q0+(qTr-q0)*t title "trial loading path 0", \
#     pk+(pTrk-pk)*t, qk+(qTrk-qk)*t title "trial loading path k"
...
#plot pc0*t, M*pc0*t*((-log(t)/log(R))**(1.0/N)) title "yield envelope 0", \
#     pc1*t, M*pc1*t*((-log(t)/log(R))**(1.0/N)) title "yield envelope 1", \
#     pc2*t, M*pc2*t*((-log(t)/log(R))**(1.0/N)) title "yield envelope 2", \
#     pc3*t, M*pc3*t*((-log(t)/log(R))**(1.0/N)) title "yield envelope 3", \
#     pc4*t, M*pc4*t*((-log(t)/log(R))**(1.0/N)) title "yield envelope 4", \
#     pc5*t, M*pc5*t*((-log(t)/log(R))**(1.0/N)) title "yield envelope 5", \
#     pc6*t, M*pc6*t*((-log(t)/log(R))**(1.0/N)) title "yield envelope 6", \
#     t*3e1, M*t*3e1 title "CSL", \
#     p0+(p1-p0)*t, q0+(q1-q0)*t title "path 1", \
#     p1+(p2-p1)*t, q1+(q2-q1)*t title "path 2", \
#     p2+(p3-p2)*t, q2+(q3-q2)*t title "path 3", \
#     p3+(p4-p3)*t, q3+(q4-q3)*t title "path 4", \
#     p4+(p5-p4)*t, q4+(q5-q4)*t title "path 5", \
#     p5+(p6-p5)*t, q5+(q6-q5)*t title "path 6"

set arrow from p0,q0 to p1,q1 head filled ls 1
set arrow from p1,q1 to p2,q2 head filled ls 1
