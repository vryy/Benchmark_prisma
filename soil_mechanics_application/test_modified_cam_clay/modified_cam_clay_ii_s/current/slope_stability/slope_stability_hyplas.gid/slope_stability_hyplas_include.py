...
##################################################################
class Model:
    def __init__( self, problem_name, path, logging=True, abs_tol=1e-10, rel_tol=1e-10 ):
        ...
        self.solver.solver.max_iter = 20
...
#                        #item.Free(AIR_PRESSURE)
    ...
    def WriteOutput( self, time ):
        ...
        print("write Gauss points results")
    ...
    def InitializeModel( self ):
        ...
        if self.material == "linear-elastic":
        ...
        elif self.material == "mohr-coulomb":
            void_ratio = 1.3
            kappa = 0.0077
            p = 55.0
            q = 46.0
            nu = 0.3
            M = 1.2
            phi = math.asin(3.0*M / (6.0+M)) # friction angle
            phi_deg = phi * (180.0/math.pi)
            E = 3.0*(1.0+void_ratio)/kappa * p * (1.0 - 2.0*nu)
            c = q - p*math.tan(phi)
            print(f"Estimated E from MCC: {E}")
            print(f"Estimated phi from MCC: {phi_deg}")
            print(f"Estimated c from MCC: {c}")
            # sys.exit(0)
            self.model_part.Properties[1].SetValue(DENSITY,       2.038735 )
            self.model_part.Properties[1].SetValue(YOUNG_MODULUS,        E ) #20000.0 )
            self.model_part.Properties[1].SetValue(POISSON_RATIO,         nu )
            self.model_part.Properties[1].SetValue(INTEGRATION_ORDER,            2 )
            self.model_part.Properties[1].SetValue(THICKNESS,            1.0 )
            self.model_part.Properties[1].SetValue(INTERNAL_FRICTION_ANGLE,       phi_deg )
            self.model_part.Properties[1].SetValue(DILATANCY_ANGLE,            phi_deg )
            self.model_part.Properties[1].SetValue(COHESION,            c )
            ...
            print("Mohr Coulomb plasticity model selected")
        elif self.material == "mohr-coulomb-ref":
            self.model_part.Properties[1].SetValue(DENSITY,       2.038735 )
            self.model_part.Properties[1].SetValue(YOUNG_MODULUS,        20000.0 )
            self.model_part.Properties[1].SetValue(POISSON_RATIO,         0.49 )
            self.model_part.Properties[1].SetValue(INTEGRATION_ORDER,            2 )
            ...
            self.model_part.Properties[1].SetValue(CONSTITUTIVE_LAW, MohrCoulomb3dImplicit() )
            print("Mohr Coulomb plasticity model selected")
        elif self.material == "mcc-s":
            self.model_part.Properties[1].SetValue(DENSITY,       2.038735 )
            self.model_part.Properties[1].SetValue(THICKNESS,            1.0 )
            self.model_part.Properties[1].SetValue(INTEGRATION_ORDER,            2 )
            self.model_part.Properties[1].SetValue(POISSON_RATIO,      0.3 )
            self.model_part.Properties[1].SetValue(VOID_RATIO,      1.788 )
            self.model_part.Properties[1].SetValue(CSL_SLOPE,     1.2 )
            ...
            # self.model_part.Properties[1].SetValue(MINIMUM_NUMBER_OF_SUB_STEPS, 2 )
            print("Modified Cam-Clay with sub-stepping model selected")
        elif self.material == "mcc-s1":
            self.model_part.Properties[1].SetValue(DENSITY,       2.038735 )
            self.model_part.Properties[1].SetValue(THICKNESS,            1.0 )
            self.model_part.Properties[1].SetValue(INTEGRATION_ORDER,            2 )
            self.model_part.Properties[1].SetValue(POISSON_RATIO,      0.3 )
            ...
            print("Modified Cam-Clay with one-step model selected")
        elif self.material == "mcc-as":
            self.model_part.Properties[1].SetValue(DENSITY,       2.038735 )
            self.model_part.Properties[1].SetValue(THICKNESS,            1.0 )
            self.model_part.Properties[1].SetValue(INTEGRATION_ORDER,            2 )
            self.model_part.Properties[1].SetValue(POISSON_RATIO,      0.3 )
            ...
            self.model_part.Properties[1].SetValue(FIX_VOID_RATIO, False )
            self.model_part.Properties[1].SetValue(CONSTITUTIVE_LAW, ClayAndSandImplicitSubstepping() )
            print("CASM with sub-stepping model selected")
        elif self.material == "casm-as":
            self.model_part.Properties[1].SetValue(DENSITY,       2.038735 )
            self.model_part.Properties[1].SetValue(THICKNESS,            1.0 )
            self.model_part.Properties[1].SetValue(INTEGRATION_ORDER,            2 )
            self.model_part.Properties[1].SetValue(POISSON_RATIO,      0.3 )
