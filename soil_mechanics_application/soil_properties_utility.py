...
class SoilPropertiesUtility:
    ...
    def __init__( self, matfile ):
        ...
        self.linearelastic3dpointer = LinearElastic3D()
        self.planestrainpointer = LinearElasticPlaneStrain()
        self.planestresspointer = LinearElasticPlaneStress()
        self.druckerprager0pointer = DruckerPrager3dImplicit_OuterEdgeMatchWithMohrCoulomb()
        self.druckerprager1pointer = DruckerPrager3dImplicit_InnerEdgeMatchWithMohrCoulomb()
        self.druckerprager2pointer = DruckerPrager3dImplicit_PlaneStrainMatchWithMohrCoulomb()
        self.druckerprager3pointer = DruckerPrager3dImplicit_UniaxialCompressionTensionMatchWithMohrCoulomb()
        self.druckerprager4pointer = DruckerPrager3dImplicit_BiaxialCompressionTensionMatchWithMohrCoulomb()
        self.druckerprager5pointer = DruckerPrager3dExplicitSubstepping_OuterEdgeMatchWithMohrCoulomb()
        self.druckerprager6pointer = DruckerPrager3dExplicitSubstepping_InnerEdgeMatchWithMohrCoulomb()
        self.druckerprager7pointer = DruckerPrager3dExplicitSubstepping_PlaneStrainMatchWithMohrCoulomb()
        self.druckerprager8pointer = DruckerPrager3dExplicitSubstepping_UniaxialCompressionTensionMatchWithMohrCoulomb()
        self.druckerprager9pointer = DruckerPrager3dExplicitSubstepping_BiaxialCompressionTensionMatchWithMohrCoulomb()
        self.druckerprager10pointer = DruckerPrager3dImplicit_UniaxialCompressionMatchWithAbaqus()
        self.druckerprager11pointer = DruckerPrager3dImplicitNoInitialStress_OuterEdgeMatchWithMohrCoulomb()
        self.druckerprager12pointer = DruckerPrager3dImplicitNoInitialStress_InnerEdgeMatchWithMohrCoulomb()
        self.druckerprager13pointer = DruckerPrager3dImplicitNoInitialStress_PlaneStrainMatchWithMohrCoulomb()
        self.druckerprager14pointer = DruckerPrager3dImplicitNoInitialStress_UniaxialCompressionTensionMatchWithMohrCoulomb()
        self.druckerprager15pointer = DruckerPrager3dImplicitNoInitialStress_BiaxialCompressionTensionMatchWithMohrCoulomb()
        self.druckerprager16pointer = DruckerPrager3dImplicitNoInitialStress_UniaxialCompressionMatchWithAbaqus()
        self.mohrcoulombpointer = MohrCoulomb3dImplicit()
        self.mohrcoulombgpointer = MohrCoulombG3dImplicit()
        self.matsuokanakaidpflowpointer = MatsuokaNakai3dImplicit_DruckerPragerFlowRule()
        self.matsuokanakaig3d = MatsuokaNakaiG3dImplicit()
#        hyplasmohrcoulombpointer = HyplasMohrCoulomb()
        self.hyplasmohrcoulombpointer = MohrCoulomb3dImplicit()
        # smoothedmohrcoulombadpointer = SmoothedMohrCoulombADExplicitLodeAngle()
        self.smoothedmohrcoulomb0pointer = SmoothedMohrCoulombImplicitExplicitLodeAngle()
        self.smoothedmohrcoulomb1pointer = SmoothedMohrCoulombImplicitExplicitLodeAngleC1()
        self.smoothedmohrcoulomb2pointer = SmoothedMohrCoulombImplicitExplicitLodeAngleC2()
        self.smoothedmohrcoulombg3dpointer = SmoothedMohrCoulombG3dImplicit()
        self.smoothedmohrcoulombg3dc1pointer = SmoothedMohrCoulombG3dImplicitC1()
        self.smoothedmohrcoulombg3dc2pointer = SmoothedMohrCoulombG3dImplicitC2()
        self.hardening_soil_cocco = HardeningSoilCoccoImplicit()
        self.hardening_soil_clama = HardeningSoilG3dImplicitClama()
        self.hardening_soil_clama_elastic_tangent = ElasticTangentConstitutiveLaw(HardeningSoilG3dImplicitClama())
        self.umat3e = Umat3e()
    ...
    def SetMaterialProperties( self, model_part, element ):
        ...
        for counter in range(0, len(integration_points) ):
            ...
            if( mat != "dummy" ): # the material name must not be dummy
                ...
                if( (material.params['model_type'] == "isotropic3d") or (material.params['model_type'] == "plane_strain") or (material.params['model_type'] == "plane_stress") or (material.params['model_type'] == "linear_elastic") ):
                    if( material.params['model_type'] == "isotropic3d" ):
                        cl_pointers.append( self.isotropic3dpointer.Clone() )
                    elif( material.params['model_type'] == "linear_elastic" ):
                        cl_pointers.append( self.linearelastic3dpointer.Clone() )
                    ...
                    youngs_moduli.append( float(material.params['youngs_modulus']) )
                    poisson_ratios.append( float(material.params['poisson_ratio']) )
                ...
                elif( "drucker_prager" in material.params['model_type'] ):
                    if( material.params['model_type'] == "drucker_prager_outer_edge_match" ):
                        cl_pointers.append( self.druckerprager0pointer.Clone() )
                    elif( material.params['model_type'] == "drucker_prager_inner_edge_match" ):
                        cl_pointers.append( self.druckerprager1pointer.Clone() )
                    elif( material.params['model_type'] == "drucker_prager_plane_strain_match" ):
                        cl_pointers.append( self.druckerprager2pointer.Clone() )
                    elif( material.params['model_type'] == "drucker_prager_uniaxial_compression_tension_match" ):
                        cl_pointers.append( self.druckerprager3pointer.Clone() )
                    elif( material.params['model_type'] == "drucker_prager_biaxial_compression_tension_match" ):
                        cl_pointers.append( self.druckerprager4pointer.Clone() )
                    elif( material.params['model_type'] == "drucker_prager_uniaxial_compression_match_with_abaqus" ):
                        cl_pointers.append( self.druckerprager10pointer.Clone() )
                    elif( material.params['model_type'] == "drucker_prager_explicit_substepping_outer_edge_match" ):
                        cl_pointers.append( self.druckerprager5pointer.Clone() )
                    elif( material.params['model_type'] == "drucker_prager_explicit_substepping_inner_edge_match" ):
                        cl_pointers.append( self.druckerprager6pointer.Clone() )
                    elif( material.params['model_type'] == "drucker_prager_explicit_substepping_plane_strain_match" ):
                        cl_pointers.append( self.druckerprager7pointer.Clone() )
                    elif( material.params['model_type'] == "drucker_prager_explicit_substepping_uniaxial_compression_tension_match" ):
                        cl_pointers.append( self.druckerprager8pointer.Clone() )
                    elif( material.params['model_type'] == "drucker_prager_explicit_substepping_biaxial_compression_tension_match" ):
                        cl_pointers.append( self.druckerprager9pointer.Clone() )
                    elif( material.params['model_type'] == "drucker_prager_outer_edge_match_no_initial_stress" ):
                        cl_pointers.append( self.druckerprager11pointer.Clone() )
                    elif( material.params['model_type'] == "drucker_prager_inner_edge_match_no_initial_stress" ):
                        cl_pointers.append( self.druckerprager12pointer.Clone() )
                    elif( material.params['model_type'] == "drucker_prager_plane_strain_match_no_initial_stress" ):
                        cl_pointers.append( self.druckerprager13pointer.Clone() )
                    elif( material.params['model_type'] == "drucker_prager_uniaxial_compression_tension_match_no_initial_stress" ):
                        cl_pointers.append( self.druckerprager14pointer.Clone() )
                    elif( material.params['model_type'] == "drucker_prager_biaxial_compression_tension_match_no_initial_stress" ):
                        cl_pointers.append( self.druckerprager15pointer.Clone() )
                    elif( material.params['model_type'] == "drucker_prager_uniaxial_compression_match_with_abaqus_no_initial_stress" ):
                        cl_pointers.append( self.druckerprager16pointer.Clone() )
                    else:
                        cl_pointers.append( self.druckerprager0pointer.Clone() )
                    youngs_moduli.append( float(material.params['youngs_modulus']) )
                    poisson_ratios.append( float(material.params['poisson_ratio']) )
                    K0_values.append( float(material.params['K0']) )
                    cohesions.append( float(material.params['cohesion']) )
                    hardening_moduli.append( float(material.params['hardening_modulus']) )
                    internal_friction_angles.append( float(material.params['internal_friction_angle']) )
                    dilatancy_angles.append( float(material.params['dilatancy_angle']) )
                    densities.append( float(material.params['density']) )
                    transition_angles.append( 0.0 )
                    apex_control_params.append( 0.0 )
                elif( "mohr_coulomb" in material.params['model_type'] ):
                    if( material.params['model_type'] == "mohr_coulomb" ):
                        cl_pointers.append( self.mohrcoulombpointer.Clone() )
                    elif( material.params['model_type'] == "hyplas_mohr_coulomb" ):
                        cl_pointers.append( self.hyplasmohrcoulombpointer.Clone() )
                    elif( material.params['model_type'] == "smoothed_mohr_coulomb" ):
                        cl_pointers.append( self.smoothedmohrcoulomb0pointer.Clone() )
                    elif( material.params['model_type'] == "smoothed_mohr_coulomb_ad" ):
                        cl_pointers.append( self.smoothedmohrcoulombadpointer.Clone() )
                    elif( material.params['model_type'] == "smoothed_mohr_coulomb_c1" ):
                        cl_pointers.append( self.smoothedmohrcoulomb1pointer.Clone() )
                    elif( material.params['model_type'] == "smoothed_mohr_coulomb_c2" ):
                        cl_pointers.append( self.smoothedmohrcoulomb2pointer.Clone() )
                    elif( material.params['model_type'] == "smoothed_mohr_coulomb_g3d" ):
                        cl_pointers.append( self.smoothedmohrcoulombg3dpointer.Clone() )
                    elif( material.params['model_type'] == "smoothed_mohr_coulomb_g3d_c1" ):
                        cl_pointers.append( self.smoothedmohrcoulombg3dc1pointer.Clone() )
                    elif( material.params['model_type'] == "smoothed_mohr_coulomb_g3d_c2" ):
                        cl_pointers.append( self.smoothedmohrcoulombg3dc2pointer.Clone() )
                    elif( material.params['model_type'] == "mohr_coulomb_g3d" ):
                        cl_pointers.append( self.mohrcoulombgpointer.Clone() )
                    youngs_moduli.append( float(material.params['youngs_modulus']) )
                    poisson_ratios.append( float(material.params['poisson_ratio']) )
                    K0_values.append( float(material.params['K0']) )
                    cohesions.append( float(material.params['cohesion']) )
                    ...
                    dilatancy_angles.append( float(material.params['dilatancy_angle']) )
                    densities.append( float(material.params['density']) )
                    if( "smoothed_mohr_coulomb" in material.params['model_type'] ):
                        transition_angles.append( float(material.params['transition_angle']) )
                        apex_control_params.append( float(material.params['apex_control_parameter']) )
                    else:
                        transition_angles.append( 0.0 )
                        apex_control_params.append( 0.0 )
                elif( "matsuoka_nakai" in material.params['model_type'] ):
                    if( material.params['model_type'] == "matsuoka_nakai_dp_flow" ):
                        cl_pointers.append( self.matsuokanakaidpflowpointer.Clone() )
                    elif( material.params['model_type'] == "matsuoka_nakai_g3d" ):
                        cl_pointers.append( self.matsuokanakaig3d.Clone() )
                    youngs_moduli.append( float(material.params['youngs_modulus']) )
                    poisson_ratios.append( float(material.params['poisson_ratio']) )
                    K0_values.append( float(material.params['K0']) )
                    cohesions.append( float(material.params['cohesion']) )
                    ...
                    transition_angles.append( 0.0 )
                    apex_control_params.append( 0.0 )
                elif( material.params['model_type'] == "umat" ):
                    ...
                    transition_angles.append( 0.0 )
                    apex_control_params.append( 0.0 )
                elif( "hardening_soil" in material.params['model_type'] ):
                    if material.params['model_type'] == "hardening_soil_cocco":
                        cl_pointers.append( self.hardening_soil_cocco.Clone() )
                    elif material.params['model_type'] == "hardening_soil_clama":
                        cl_pointers.append( self.hardening_soil_clama.Clone() )
                    elif material.params['model_type'] == "hardening_soil_clama_elastic_tangent":
                        cl_pointers.append( self.hardening_soil_clama_elastic_tangent.Clone() )
                    cl = cl_pointers[-1]
                    # print(material.params['material_params'])
                    tmp = ast.literal_eval(material.params['material_params'])
                    mat_params = Vector(len(tmp))
                    for i in range(0, len(tmp)):
                        mat_params[i] = float(tmp[i])
                    cl.SetValue(MATERIAL_PARAMETERS, mat_params, model_part.ProcessInfo)
