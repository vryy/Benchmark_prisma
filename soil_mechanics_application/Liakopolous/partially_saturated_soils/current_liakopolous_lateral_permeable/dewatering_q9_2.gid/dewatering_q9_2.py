...
import dewatering_q9_2_include as simulation_include
try:
    from dewatering_q9_2_include import *
    all_modules_are_imported_successfully = True
except Exception as e:
    all_modules_are_imported_successfully = False
##################################################################
model_name_ = 'dewatering_q9_2'
##################################################################
###  SIMULATION  #################################################
...
def main(output=True, logging=True, dt=1.0, num_steps=7200, p=0.0, analysis_type=1, dissipation_radius=0.1,apply_top_air_pressure=False, thickness=0.2):

    model_virgin = simulation_include.Model(model_name_,os.getcwd()+"/",os.getcwd()+"/virgin_results/",logging=False)
    model_virgin.InitializeModel(thickness=thickness)
    ...
    model = simulation_include.Model(model_name_,os.getcwd()+"/",os.getcwd()+"/",logging=logging,analysis_type=analysis_type,dissipation_radius=dissipation_radius)
    model.InitializeModel(thickness=thickness)
    ...
    # As a consequence, the water flow will differ from the 1D curve. Only when the AIR_PRESSURE
    # is fixed totally, the 1D curve can be matched closedly.
    # Furthermore, in the plane strain analysis, the out-of-plane lateral nodes are not fixed, leading
    # to slightly different test result to 3D case
    ...
    for step in range(0, num_steps):
        ...
        print("~~~~~~~~Load step %e" % (time), flush=True)
    ...
    #print(model.model_part.Nodes[1].GetSolutionStepValue(DISPLACEMENT_Y))

    return model

def test():
    model = main(logging=False, output=False, dt=1.0, num_steps=100, p=0.0, analysis_type=1, dissipation_radius=0.1, apply_top_air_pressure=False)
    ...
    wf2 = wf[1]*60.0/0.01
    print("%.16e" % (wf2))
    ref_wf2 = -2.4414066654827822e-02 # for num_steps = 100
...
if __name__ == '__main__':
    if len(sys.argv) > 1:
        ...
        model = main(logging=True, output=False, thickness=0.2)
