...
class Model:
    ...
    def InitializeModel( self ):
        ...
        ##
        mat_params[11] = 0.0    # not used
        mat_params[12] = 0.0    # not used
        mat_params[13] = 0.0    # not used
        ##
        mat_params[14] = 6.5e4  # Eiref
        mat_params[15] = 1.447  # alpha
        mat_params[16] = 0.0    # not used
        mat_params[17] = 75215.757 # H
        # mat_params[14] = 0. # not perform internal parameters optimization
        # mat_params[14] = 1. # perform internal parameters optimization
        ##
        self.model_part.Properties[1].SetValue(DENSITY,         0.0 )
        self.model_part.Properties[1].SetValue(THICKNESS,            1.0 )
        self.model_part.Properties[1].SetValue(MATERIAL_PARAMETERS, mat_params)
        self.model_part.Properties[1].SetValue(CONSTITUTIVE_LAW, HardeningSoilCoccoImplicit() )
        print("Hardening soil model selected, description: Leonardo Cocco")
        ##################################################################
        ## ACTIVATION ####################################################
        ##################################################################
