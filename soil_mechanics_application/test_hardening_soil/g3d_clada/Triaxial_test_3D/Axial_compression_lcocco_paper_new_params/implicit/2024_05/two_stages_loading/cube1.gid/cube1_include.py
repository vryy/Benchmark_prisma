...
class Model:
    def __init__( self, problem_name, path, results_path, logging = True ):
        ...
        (self.solver.solver).max_iter = 50 #20 #control the maximum iterations of Newton Raphson loop
    ...
    def InitializeModel( self ):
        ...
        # self.model_part.Properties[1].SetValue(YOUNG_MODULUS,      6.e4 )
        # self.model_part.Properties[1].SetValue(POISSON_RATIO,          0.25 )
        ...
        ##
        mat_params[11] = 0.0    # sigma tension
        mat_params[12] = 6.5e4  # E0ref
        mat_params[13] = 0.0    # gamma_0.7
        mat_params[14] = 6.5e4  # Eiref
        mat_params[15] = 1.447  # alpha
        mat_params[16] = 9.0e4 / 3.0e4    # Ks/Kc (= Eurref / Eoedref)
        # mat_params[16] = 8.0    # Ks/Kc
        ##
        self.model_part.Properties[1].SetValue(DENSITY,         0.0 )
        self.model_part.Properties[1].SetValue(THICKNESS,            1.0 )
        ...
        self.model_part.Properties[1].SetValue(CONSTITUTIVE_LAW, HardeningSoilG3dImplicitClada() )
        self.model_part.Properties[1].SetValue(YIELD_TOLERANCE, 1e-8 )
        print("Hardening soil model selected, description: CLADA")
        ##################################################################
        ## ACTIVATION ####################################################
        ##################################################################
