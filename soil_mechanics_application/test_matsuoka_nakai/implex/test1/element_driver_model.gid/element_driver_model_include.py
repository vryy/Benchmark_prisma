...
##################################################################
class Model:
    def __init__( self, problem_name, path, results_path, logging=True, dilatancy_angle=15.0 ):
        ...
        self.analysis_parameters['analysis_type'] = 3
        self.analysis_parameters['dissipation_radius'] = 1.0
        ...
        #self.analysis_parameters['builder_and_solver_type'] = 'residual-based block'
        ...
        ## generating solver
        #import freezing_soil_solver_advanced
        #self.solver = freezing_soil_solver_advanced.FreezingSoilSolver(self.model_part, self.domain_size, self.abs_tol,self.rel_tol)
        #freezing_soil_solver_advanced.AddVariables( self.model_part )
        #self.solver.SetDampingFactor(1.0)
        ##################################################################
        ## READ MODELPART ################################################
        ##################################################################
        #reading a model
        ...
        #for node in self.model_part.Nodes:
        #    node.SetSolutionStepValue(TEMPERATURE, 1.0)
        #    node.Fix( TEMPERATURE )
        #freezing_soil_solver_advanced.AddDofs( self.model_part )
        ##################################################################
        ...
        #self.solver.SetMaxIterationNumber(50)
        (self.solver.solver).SetEchoLevel(2)
        (self.solver.solver).max_iter = 500 #control the maximum iterations of Newton Raphson loop
    ...
    def WriteOutput( self, time ):
        ...
        self.gid_io.PrintOnGaussPoints(OVERCONSOLIDATION_RATIO, self.model_part, time)
        self.gid_io.PrintOnGaussPoints(PRECONSOLIDATION_PRESSURE, self.model_part, time)
        self.gid_io.PrintOnGaussPoints(PRESTRESS_FACTOR, self.model_part, time)
        ...
        self.gid_io.PrintOnGaussPoints(PRESSURE_P, self.model_part, time)
        self.gid_io.PrintOnGaussPoints(PRESSURE_Q, self.model_part, time)
#        self.gid_io.PrintOnGaussPoints(MOBILIZED_COHESION, self.model_part, time)
#        self.gid_io.PrintOnGaussPoints(MOBILIZED_DILATANCY_ANGLE, self.model_part, time)
#        self.gid_io.PrintOnGaussPoints(MOBILIZED_FRICTION_ANGLE, self.model_part, time)
    ...
    def InitializeModel( self ):
        ...
        ##################################################################
        # cohesion
        #T = -1 C, cohesion = 0.6967738547592557 MPa
        #T = -2 C, cohesion = 1.1762186812748483 MPa
        #T = -5 C, cohesion = 1.9483116486506074 MPa
        #T = -10 C, cohesion = 2.8636154083621923 MPa
        # dilatancy 11.14
        # dilatancy 8.85
        #set material parameters
        self.model_part.Properties[1].SetValue(THICKNESS, 1.0 )
        self.model_part.Properties[1].SetValue(DENSITY,        1.0 )
        self.model_part.Properties[1].SetValue(YOUNG_MODULUS,      100e6 )
        self.model_part.Properties[1].SetValue(POISSON_RATIO,          0.333333 )
        self.model_part.Properties[1].SetValue(INTERNAL_FRICTION_ANGLE,          30 ) # 30
        self.model_part.Properties[1].SetValue(DILATANCY_ANGLE, self.dilatancy_angle) #0.0 # 15 # 30
        self.model_part.Properties[1].SetValue(COHESION,         0 )#0  #1e3
        self.model_part.Properties[1].SetValue(ISOTROPIC_HARDENING_MODULUS,         0.0)
        # self.model_part.Properties[1].SetValue(VISCOSITY,         1)
        self.model_part.Properties[1].SetValue(INTEGRATION_ORDER,            2 )
#        self.model_part.Properties[1].SetValue(CONSTITUTIVE_LAW, DruckerPrager3dImplicit_OuterEdgeMatchWithMohrCoulomb() )
#        self.model_part.Properties[1].SetValue(CONSTITUTIVE_LAW, MohrCoulomb3dImplex() )
#        self.model_part.Properties[1].SetValue(CONSTITUTIVE_LAW, MohrCoulomb3dImplicitEv() )
#        self.model_part.Properties[1].SetValue(CONSTITUTIVE_LAW, MohrCoulomb3dImplicit() )
#        self.model_part.Properties[1].SetValue(CONSTITUTIVE_LAW,MatsuokaNakai3dImplex() )
#        self.model_part.Properties[1].SetValue(CONSTITUTIVE_LAW,MatsuokaNakai3dImplex_DruckerPragerFlowRule() )
#        self.model_part.Properties[1].SetValue(CONSTITUTIVE_LAW,MatsuokaNakai3dImplicit() )
        self.model_part.Properties[1].SetValue(CONSTITUTIVE_LAW,MatsuokaNakai3dImplicit_DruckerPragerFlowRule() )
#        self.model_part.Properties[1].SetValue(CONSTITUTIVE_LAW, MohrCoulombFrozenMobilizedFrictionHardeningImplex() )
#        self.model_part.Properties[1].SetValue(CONSTITUTIVE_LAW, MatsuokaNakaiFrozenMobilizedFrictionHardeningImplex() )
