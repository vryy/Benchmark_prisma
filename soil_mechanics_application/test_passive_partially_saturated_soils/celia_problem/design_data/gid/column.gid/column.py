...
##### all rights reserved                                    #####
##################################################################
## This file is generated on Tue Jan 27 04:32:11 +07 2026
##################################################################
import sys
import os
import math
...
import column_include as simulation_include
try:
    from column_include import *
...
def SetMaterialProperties(elem):
    ...
    elem.SetValue(DENSITY,                  density)
    elem.SetValue(DENSITY_WATER,            density_water)
    elem.SetValue(POROSITY,                 n)
    elem.SetValue(PERMEABILITY_WATER,       Ks)
    aux_util.SetValue(SWCC_LAW, VanGenuchtenSWCC(s1, s2, pr, Smin, Smax), elem)
    aux_util.SetValue(RELATIVE_PERMEABILITY_WATER_LAW, HaverkampRelativePermeabilityWaterLaw(A, gamma, density_water*g, s1, s2, pr, Smin, Smax), elem)
...
def main(output=True, logging=True, total_time=360.0, delta_time=1.0, \
    ...
    p_bottom = -density_water*g*61.5
    for node in model.model_part.Nodes:
        node.SetSolutionStepValue(WATER_PRESSURE, p_bottom)
        node.SetSolutionStepValue(WATER_PRESSURE_EINS, p_bottom)
        node.SetSolutionStepValue(WATER_PRESSURE_NULL, p_bottom)

    p_top = -density_water*g*20.7
    for node in top_nodes:
        node.SetSolutionStepValue(WATER_PRESSURE, p_top)
        node.SetSolutionStepValue(WATER_PRESSURE_EINS, p_top)
        node.SetSolutionStepValue(WATER_PRESSURE_NULL, p_top)
    ...
    if logging:
        ...
        ifile.write("depth\tpressure_head\n")

        values = []
        for node in model.model_part.Nodes:
            if abs(node.X0) < tol and abs(node.Y0) < tol:
                ph = node.GetSolutionStepValue(WATER_PRESSURE) / (density_water*g)
                values.append([node.Z0, ph])
...
# def test():
#     model = main(logging=False, output=False, dt=10.0, num_steps=10, p=0.0, analysis_type=1, dissipation_radius=0.9)
...
#     wf2 = wf[1]*60.0/0.01
#     print("%.16e" % (wf2))
#     ref_wf2 = -3.3377229875174269e-01
...
def tag():
    if all_modules_are_imported_successfully:
        return "passive-partially-saturated"
    else:
        return ""
