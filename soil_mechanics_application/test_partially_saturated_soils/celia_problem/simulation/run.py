...
def SetMaterialProperties(elem):
    ...
    aux_util.SetValue(GAS_LAW, IdealGasLaw(1.0, 1.0), elem)
...
def test():
    model = main(logging=False, output=False, total_time=360.0, delta_time=1.0, analysis_type=4, dissipation_radius=0.5)

    tol = 1.0e-6
    for node in model.model_part.Nodes:
        if abs(node.X0) < tol and abs(node.Y0) < tol and abs(node.Z0 - 20.0) < tol:
            mon_node = node
    ...
    print("ph: %.16e, diff: %.6e" % (ph, ph - ref_ph))
