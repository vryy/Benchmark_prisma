##################################################################
# CDPM2, Single element test - pure shear
...
def Record(ifile, time, model_part):
    disp = model_part.Nodes[3].GetSolutionStepValue(DISPLACEMENT_X)
    reac = model_part.Nodes[3].GetSolutionStepValue(REACTION_X)
...
def main(logging=True, output=True, nsteps=1000, delta_time=1e-4, total_disp=2e-3):
    ...
    model.model_part.Nodes[3].Fix(DISPLACEMENT_X) # prescribed dof
    ...
    for i in range(0, nsteps):
        time += delta_time
        model.model_part.Nodes[3].SetSolutionStepValue(DISPLACEMENT_X, time*total_disp)
...
def test():
    model = main(logging=False, output=False, nsteps=200)

    reac = model.model_part.Nodes[3].GetSolutionStepValue(REACTION_X)
    ref_reac = 1.4899887284005827e+03
...
if __name__ == '__main__':
    if len(sys.argv) > 1:
        ...
        main(logging=True, output=False, nsteps=1000, delta_time=1e-4)
