...
class Model:
    ...
    def CreateModel(self, mpatch, mpatch_sub):
        mpatch_util = MultiPatchUtility()
        element_name = "ReissnerMindlinElasticLinearShellElementDFadDFadBezier2D3"

        mpatch_mp = MultiMultiPatchModelPart2D()
        mpatch_mp.AddMultiPatch(mpatch)
        mpatch_mp.AddMultiPatch(mpatch_sub)
        ...
        for patch_ptr in mpatch.Patches():
            ...
            sub_patch_ptr = mpatch_sub[patch.Id]
            sub_patch = sub_patch_ptr.GetReference()
            sub_patch.LayerIndex = 1

            ## add volume elements
            last_elem_id = mpatch_util.GetLastElementId(model_part)
            elems = mpatch_mp.AddElements([patch, sub_patch], element_name, last_elem_id+1, prop)
            # print(len(elems))
    ...
    def ExtractForce(model_part, x_ref=None, export=True, filename="force.txt"):
        ...
        if x_ref == None:
            for elem in model_part.Elements:
                ...
                for i in range(0, len(coords)):
                    all_values.append([coords[i], n[i], m[i], q[i]])
        else:
            for elem in model_part.Elements:
                coords = elem.CalculateOnIntegrationPoints(INTEGRATION_POINT_GLOBAL_IN_REFERENCE_CONFIGURATION, model_part.ProcessInfo)
                m = elem.CalculateOnIntegrationPoints(LOCAL_BENDING_MOMENT, model_part.ProcessInfo)
        ...
        if export:
            ifile = open(filename, "w")
            ifile.write("%-*s%-*s%-*s%-*s%-*s%-*s%-*s%-*s%-*s%-*s%-*s%-*s%s\n" % (20, "x", 20, "y", 20, "z", 20, "n00", 20, "n01", 20, "n10", 20, "n11", 20, "m00", 20, "m01", 20, "m10", 20, "m11", 20, "q0", "q1"))
            for v in all_values:
                # print(v[1])
                ifile.write("%-*.12e%-*.12e%-*.12e%-*.12e%-*.12e%-*.12e%-*.12e%-*.12e%-*.12e%-*.12e%-*.12e%-*.12e%.12e\n" % (20, v[0][0], 20, v[0][1], 20, v[0][2], 20, v[1][0], 20, v[1][1], 20, v[1][2], 20, v[1][3], 20, v[2][0], 20, v[2][1], 20, v[2][2], 20, v[2][3], 20, v[3][0], v[3][1]))
            ifile.close()
        return all_values
    ...
    def ConvertForce(values, export=True, filename='force_polar.txt'):
        ...
        for v in values:
            ...
            z = v[0][2]
            a = math.atan2(y, z)
            values_with_ang.append([a, v[1][0], v[1][1], v[1][2], v[1][3], v[2][0], v[2][1], v[2][2], v[2][3], v[3][0], v[3][1]])
        sorted_values_with_ang = sorted(values_with_ang, key = lambda x: x[0])
        if export:
            ifile = open(filename, "w")
            ifile.write("%-*s%-*s%-*s%-*s%-*s%-*s%-*s%-*s%-*s%-*s%s\n" % (20, "a", 20, "n00", 20, "n01", 20, "n10", 20, "n11", 20, "m00", 20, "m01", 20, "m10", 20, "m11", 20, "q0", "q1"))
            for v in sorted_values_with_ang:
                ifile.write("%-*.12e%-*.12e%-*.12e%-*.12e%-*.12e%-*.12e%-*.12e%-*.12e%-*.12e%-*.12e%.12e\n" % (20, v[0], 20, v[1], 20, v[2], 20, v[3], 20, v[4], 20, v[5], 20, v[6], 20, v[7], 20, v[8], 20, v[9], v[10]))
            ifile.close()
        return sorted_values_with_ang
    ...
    def Run(self, output=True, logging=True, disp=None, rot=None):
        ...
        mpatch = self.CreateMultiPatch(length_bar, radius_bar, order=self.order, mode=self.mode)
        mpatch = self.Refine(mpatch, self.nsampling)
        mpatch_sub = self.CreateMultiPatch(length_bar, radius_bar, order=self.order-1, mode=self.mode)
        mpatch_sub = self.Refine(mpatch_sub, self.nsampling)
        ...
        patch_elems, mpatch_mp = self.CreateModel(mpatch, mpatch_sub)
        model_part = mpatch_mp.GetModelPart()
        ...
        if output:
            ...
            patch_orig_elems, mpatch_mp_orig = self.CreateModel(mpatch_orig, mpatch_sub_orig)
            ...
            mpatch_mp_orig.SynchronizeBackward(0, DISPLACEMENT)
            mpatch_mp_orig.SynchronizeBackward(1, ROTATION)
