...
class Model:
    ...
    def InitializeModel( self ):
        ...
        mat_params[8] = 0.2     # fC
        mat_params[9] = 4.0     # kappa
        mat_params[10] = 1.0     # rfac
        self.model_part.Properties[1].SetValue(CONSTITUTIVE_LAW, GradientEnhancedDamagePlasticityGTNLaw() )
        self.model_part.Properties[1].SetValue(GRADIENT_DAMAGE_PENALIZATION, 1.0 )
        self.model_part.Properties[1].SetValue(GRADIENT_PLASTICITY_PENALIZATION, 1.0 )
        self.model_part.Properties[1].SetValue(LOCAL_NONLOCAL_DAMAGE_PENALIZATION, 1.0 )
        self.model_part.Properties[1].SetValue(LOCAL_NONLOCAL_PLASTICITY_PENALIZATION, 1.0 )
        ## enable this to enable local GTN
        # self.model_part.Properties[1].SetValue(GRADIENT_DAMAGE_PENALIZATION, 1.0 )
        # self.model_part.Properties[1].SetValue(GRADIENT_PLASTICITY_PENALIZATION, 1.0 )
        # self.model_part.Properties[1].SetValue(LOCAL_NONLOCAL_DAMAGE_PENALIZATION, 0.0 )
        # self.model_part.Properties[1].SetValue(LOCAL_NONLOCAL_PLASTICITY_PENALIZATION, 0.0 )
        ### end of enable this
        self.model_part.Properties[1].SetValue(MATERIAL_PARAMETERS, mat_params )
        hpoints = [375.0, 0.0, 375.0, 100.0]
        npoints = int(len(hpoints)/2)
