...
class Model:
    def __init__( self, problem_name, path, results_path, logging ):
        ...
        self.analysis_parameters['calculate_strain_energy'] = logging
    ...
    def InitializeModel( self ):
        ...
        # self.model_part.Properties[1].SetValue(POISSON_RATIO,          0.33 )
        ...
        # self.model_part.Properties[1].SetValue(DENSITY,         0.0 )
        # self.model_part.Properties[1].SetValue(YOUNG_MODULUS,      210.0e3 )
        # self.model_part.Properties[1].SetValue(POISSON_RATIO,          0.33 )
        # self.model_part.Properties[1].SetValue(THICKNESS, 0.01 )
        # self.model_part.Properties[1].SetValue(ERVE_NUMBER_OF_DOMAIN,          20 ) # n
        # # self.model_part.Properties[1].SetValue(ERVE_YIELD_LIMIT_FOR_DAMAGE,          0.5e-6 ) # r
        # self.model_part.Properties[1].SetValue(YIELD_LIMIT_FOR_DAMAGE,          0.5e-3 ) # r
        # self.model_part.Properties[1].SetValue(ERVE_RATE_LIMIT,          0.018 ) # k
        # self.model_part.Properties[1].SetValue(DAMAGE_LIMIT,          0.999 ) # dmax
        # self.model_part.Properties[1].SetValue(ERVE_STIFFNESS_CONTROL_OPTION,    2 ) # use implicit stiffness
        # self.model_part.Properties[1].SetValue(ERVE_DELTA_PSI,    1.0e-8 ) # parameter to estimate the implicit stiffness
        # self.model_part.Properties[1].SetValue(CONSTITUTIVE_LAW, ERVEDamage_PlaneStrain() )
        # # util = PhaseFieldUtility()
        # dlaw = SquareDamageLaw()
        # DamageLaw().Assign(DAMAGE_LAW, dlaw, self.model_part.Properties[1])
        # # util.SetValue(DAMAGE_LAW, dlaw, self.model_part.Properties[1])
        # print("ERVEDamage model is selected")
        ...
        self.model_part.Properties[1].SetValue(POISSON_RATIO,          0.33 )
        self.model_part.Properties[1].SetValue(THICKNESS, 0.01 )
        self.model_part.Properties[1].SetValue(CONSTITUTIVE_LAW, GradientEnhancedDamageViscoplasticityRateLimitLaw() )
        self.model_part.Properties[1].SetValue(GRADIENT_DAMAGE_PENALIZATION, 1.0 ) # beta_d
        self.model_part.Properties[1].SetValue(GRADIENT_PLASTICITY_PENALIZATION, 1.0 ) # beta_p
        self.model_part.Properties[1].SetValue(LOCAL_NONLOCAL_DAMAGE_PENALIZATION, 1.0 ) # alpha_d
        self.model_part.Properties[1].SetValue(LOCAL_NONLOCAL_PLASTICITY_PENALIZATION, 1.0 ) # alpha_p
        self.model_part.Properties[1].SetValue(YIELD_LIMIT_FOR_DAMAGE, 0.5e-3 ) # r_d
        # self.model_part.Properties[1].SetValue(TENSILE_STRENGTH, 2.0e2 ) # r_p # damage too early
        # self.model_part.Properties[1].SetValue(TENSILE_STRENGTH, 1.0e2 ) # r_p # going down strangely
        # self.model_part.Properties[1].SetValue(TENSILE_STRENGTH, 5.0e1 ) # r_p # going down strangely
        # self.model_part.Properties[1].SetValue(TENSILE_STRENGTH, 3.0e1 ) # r_p # going down and going horizontal downward
        self.model_part.Properties[1].SetValue(TENSILE_STRENGTH, 2.0e1 ) # r_p # hardening and then softening  slightly
        # self.model_part.Properties[1].SetValue(TENSILE_STRENGTH, 1.0e1 ) # r_p # go horizontal
        # self.model_part.Properties[1].SetValue(TENSILE_STRENGTH, 2.0 ) # r_p #  go horizontal
        self.model_part.Properties[1].SetValue(ISOTROPIC_HARDENING_MODULUS, 1.0 ) # KH

        self.model_part.Properties[1].SetValue(VISCOSITY, 10.0 ) # mu
        self.model_part.Properties[1].SetValue(RATE_SENSITIVITY, 1.0 )

        dlaw = ExponentialDamageLaw()
        DamageLaw().Assign(DAMAGE_LAW, dlaw, self.model_part.Properties[1])
        print("GradientEnhancedDamageViscoplasticityRateLimitLaw is selected")
        ##################################################################
        ## ACTIVATION ####################################################
        ##################################################################
