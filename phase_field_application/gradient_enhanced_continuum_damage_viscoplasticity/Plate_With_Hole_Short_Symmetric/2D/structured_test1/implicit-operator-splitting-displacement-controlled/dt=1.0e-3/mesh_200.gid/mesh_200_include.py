...
class Model:
    ...
    def InitializeModel( self ):
        ...
        self.model_part.Properties[1].SetValue(YOUNG_MODULUS,      18.0e3 )
        self.model_part.Properties[1].SetValue(POISSON_RATIO,          0.2 )
        self.model_part.Properties[1].SetValue(THICKNESS, 1.0 )
        self.model_part.Properties[1].SetValue(CONSTITUTIVE_LAW, GradientEnhancedDamageViscoplasticityRateLimitOpsLaw() )
        ...
        self.model_part.Properties[1].SetValue(LOCAL_NONLOCAL_DAMAGE_PENALIZATION, 1.0e1 ) # alpha_d
        self.model_part.Properties[1].SetValue(LOCAL_NONLOCAL_PLASTICITY_PENALIZATION, 1.0 ) # alpha_p
        self.model_part.Properties[1].SetValue(YIELD_LIMIT_FOR_DAMAGE, 1.0e-2 ) # r_d
        self.model_part.Properties[1].SetValue(TENSILE_STRENGTH, 2.0e1 ) # r_p
        self.model_part.Properties[1].SetValue(ISOTROPIC_HARDENING_MODULUS, 1.0e3 ) # KH

        ## working one
        # self.model_part.Properties[1].SetValue(VISCOSITY, 1.0 ) # mu
        # self.model_part.Properties[1].SetValue(RATE_SENSITIVITY, 0.5) # 2.0 ) # 1.0 # ee
        ...
        print("GradientEnhancedDamageViscoplasticityRateLimitOpsLaw is selected")
        ##################################################################
        ## ACTIVATION ####################################################
        ##################################################################
