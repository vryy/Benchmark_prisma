...
class Model:
    def __init__( self, problem_name, path, results_path, logging=True ):
        ...
        self.analysis_parameters['solution_strategy'] = "arc_length_load_control"
        self.analysis_parameters['arc_length_constraint_type'] = "cylinder-displacement"
        self.analysis_parameters['arc_length_load_scale_factor'] = 1.0
        self.analysis_parameters['arc_length_radius'] = 0.0
    ...
    def AddDofsForNodes(self, nodes):
        ...
        for node in nodes:
            node.AddDof(EQUIVALENT_STRAIN, PRESSURE)
    ...
    def AddVariables(self, model_part):
        ...
        self.model_part.AddNodalSolutionStepVariable( EQUIVALENT_STRAIN )
    ...
    def WriteOutput( self, time ):
        ...
        self.gid_io.PrintOnGaussPoints(DAMAGE, self.model_part, time)
        print("gauss point DAMAGE written")
    ...
    def InitializeModel( self ):
        ...
        ##################################################################
        # material parameters from Gutierrez paper
        self.model_part.Properties[1].SetValue(DENSITY,         0.0 )
        self.model_part.Properties[1].SetValue(YOUNG_MODULUS,      35.0e3 )
        self.model_part.Properties[1].SetValue(POISSON_RATIO,          0.2 )
        self.model_part.Properties[1].SetValue(THICKNESS,            100.0 )
        self.model_part.Properties[1].SetValue(K_PARAMETER,            10.0 )
        self.model_part.Properties[1].SetValue(ALPHA_PARAMETER,            0.96 )
        self.model_part.Properties[1].SetValue(BETA_PARAMETER,            100.0 )
        self.model_part.Properties[1].SetValue(KAPPA_0,            6.0e-5 )
        self.model_part.Properties[1].SetValue(CONSTITUTIVE_LAW, StrainBasedGradientDamageLaw1_PlaneStress() )
        # self.model_part.Properties[1].SetValue(CONSTITUTIVE_LAW, StrainBasedGradientDamageLaw1_PlaneStrain() )
        # self.model_part.Properties[1].SetValue(CONSTITUTIVE_LAW, StrainBasedGradientDamageIsotropicLaw_PlaneStrain() )
        self.model_part.Properties[1].SetValue(GRADIENT_DAMAGE_ORDER,            2 )
        self.model_part.Properties[1].SetValue(LENGTH_SCALE,            math.sqrt(2.0) )
        print("Law1 model selected for bulk, normal weight concrete")
        self.model_part.Properties[2].SetValue(DENSITY,         0.0 )
        self.model_part.Properties[2].SetValue(YOUNG_MODULUS,      1.0e6 )
        self.model_part.Properties[2].SetValue(POISSON_RATIO,          0.2 )
        self.model_part.Properties[2].SetValue(THICKNESS,            100.0 )
        self.model_part.Properties[2].SetValue(CONSTITUTIVE_LAW, PlaneStress() )
        print("Linear elastic model selected for Support, description: Bulk")
        ##################################################################
        ## ACTIVATION ####################################################
        ##################################################################
