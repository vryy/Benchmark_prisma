...
class Model:
    ...
    def AddVariables(self, model_part):
        ...
        model_part.AddNodalSolutionStepVariable(EQUIVALENT_STRAIN)
    ...
    def InitializeModel( self ):
        ...
        nu = 0.33
        ft = 5.0        # MPa
        Gf = 200e-6     # 200 N/m, high-strength concrete
        ls = 3e-1
        e0 = ft / E
        ef = Gf / (ls*ft)
        # E  = 35000
        ...
        self.model_part.Properties[1].SetValue(YOUNG_MODULUS,      E ) # 200.0e3 )
        self.model_part.Properties[1].SetValue(POISSON_RATIO,         nu ) # 0.33 )
        ...
        self.model_part.Properties[1].SetValue(LOCAL_ERROR_TOLERANCE, 0.0 )
        self.model_part.Properties[1].SetValue(FRACTURE_ENERGY, Gf )
        self.model_part.Properties[1].SetValue(TENSILE_STRENGTH, ft )
        self.model_part.Properties[1].SetValue(LENGTH_SCALE, ls )
        self.model_part.Properties[1].SetValue(INTEGRATION_ORDER, 3 )
        self.model_part.Properties[1].SetValue(GRADIENT_DAMAGE_ORDER, 2 )
        print("Isotropic damage model selected for Bulk")
        ##################################################################
        ## ACTIVATION ####################################################
        ##################################################################
