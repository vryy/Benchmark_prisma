...
class Model:
    def __init__( self, problem_name, path, results_path, logging=True ):
        ...
        (self.solver.solver).max_iter = 1000 #control the maximum iterations of Newton Raphson loop
    ...
    def AddDofsForNodes(self, nodes):
        ...
        for node in nodes:
            node.AddDof(DAMAGE_FUNCTION_VARIABLE, DAMAGE_FUNCTION_REACTION_VARIABLE)
    ...
    def AddVariables(self, model_part):
        ...
        model_part.AddNodalSolutionStepVariable(DAMAGE_FUNCTION_VARIABLE)
        model_part.AddNodalSolutionStepVariable(DAMAGE_FUNCTION_REACTION_VARIABLE)
    ...
    def WriteOutput( self, time ):
        ...
        self.gid_io.WriteNodalResults(DAMAGE_FUNCTION_VARIABLE, time, 0)
        print("nodal DAMAGE_FUNCTION_VARIABLE written")
        ...
        self.gid_io.PrintOnGaussPoints(DAMAGE_YIELD_FUNCTION, self.model_part, time)
        print("gauss point DAMAGE_YIELD_FUNCTION written")
    ...
    def InitializeModel( self ):
        ...
        self.model_part.Properties[1].SetValue(YOUNG_MODULUS,      200.0e3 )
        ...
        self.model_part.Properties[1].SetValue(GRADIENT_DAMAGE_PENALIZATION, 0.1 ) # beta
        self.model_part.Properties[1].SetValue(YIELD_LIMIT_FOR_DAMAGE, 1.0e-3 ) # r
        self.model_part.Properties[1].SetValue(PRIMAL_DUAL_REGULARIZED_FACTOR, 1.0e-2 ) # c
        self.model_part.Properties[1].SetValue(DAMAGE_KS_REGULARIZED_FACTOR, 2.0e4 ) # eta
        self.model_part.Properties[1].SetValue(INITIAL_PENALTY, 1.0e5 ) # rho_c
        print("Linear elastic model selected for Bulk, description: plane strain")
        ##################################################################
        ## ACTIVATION ####################################################
        ##################################################################
